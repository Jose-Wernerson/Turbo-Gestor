-- Execute este SQL no Supabase SQL Editor para corrigir contas existentes

-- 1. Primeiro, adicionar as colunas se não existirem (do add-plan-limits.sql)
...
-- 3. Criar função para atualizar contadores automaticamente
...
-- 5. IMPORTANTE: Atualizar TODAS as contas existentes que não têm trial_ends_at e não têm assinatura
-- Isso dá 7 dias de trial para todas as contas antigas
UPDATE oficinas 
SET 
  trial_ends_at = NOW() + INTERVAL '7 days',
  plano = CASE 
    WHEN plano IS NULL OR plano = '' OR LOWER(plano) = 'free' THEN 'basico'
    ELSE plano
  END
